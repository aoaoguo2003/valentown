...
出口 ① 依赖 ``ToolSpec.ends_turn``，而它的判据是**占不占用游戏时间**：
...
def _record(scratchpad, context, *, tool, args, ok, reason, observation, ends_turn=None):
...
def run_decision_loop(agent, *, internal_state, triggers, day_number, time_text,
    ...
    for step in range(max_steps):
        ...
        # 查同一个货架和同一个余额上，一个动作都没做出来。
        #
        # ⚠️ 判据是 ``cacheable_within_turn`` 而**不是** ``read_only``。
        # 没有副作用 ≠ 答案不会变：七个居民是并发决策的，Emma 第一步查到
        # "还剩 1"，Mia 的线程在这几十秒里买走了它，Emma 第三步再查——
        # 缓存会告诉她"还剩 1"。世界没错（buy 那把原子锁拦得住超卖），
        # 但她被主动告知了一件假事，还失去了改主意的机会。见 tools/base.py。
        ...
        if result["ok"] and spec.ends_turn:
        ...
        _record(scratchpad, trace,
                ...
                ends_turn=spec.ends_turn)

        if result["ok"] and spec.ends_turn:                  # ① 正常收敛
